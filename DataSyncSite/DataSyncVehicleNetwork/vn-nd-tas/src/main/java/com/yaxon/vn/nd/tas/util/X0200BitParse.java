...
import com.yaxon.vn.nd.tbp.si.StatusConstants;
...
public class X0200BitParse {
    ...
    public static List<String> parseAlarm(long alarm) {
        ...
        for (int i = 0; i < 32; i++) {
            if ((alarm & (1 << i)) != 0) {
                ...
                logger.debug("alarmBit " + i + " alarmExtra " + at.getValue());
            }
        }
        return alarmList;
    }
     ...
     * 0200 报警信息	F2
     * @param code
     * @return 报警信息列表 附加的 1D02终端
     */
    public static List<String> getVdrAlarmLists(long code) {
        List<String> alarmsList = Lists.newArrayList();
        if ((code & 1) != 0) {
            alarmsList.add("CAN1状态（PCAN）故障");
        } else if ((code & 2) != 0) {
            alarmsList.add("CAN2状态（BCAN）故障");
        } else if ((code & 4) != 0) {
            alarmsList.add("CAN3状态（外接CAN）故障");
        } else if ((code & 8) != 0) {
            alarmsList.add("CAN4状态（诊断CAN）故障");
        } else if ((code & 16) != 0) {
            alarmsList.add("油量液位状态故障");
        } else if ((code & 32) != 0) {
            alarmsList.add("熄火滑行状态故障");
        }
        return alarmsList;
    }
...
/*    public static void main(String[] args) {
        List a = getStateDesp(511);
        System.out.println(a);
        long aa = rightMoveBit(511, 8);
