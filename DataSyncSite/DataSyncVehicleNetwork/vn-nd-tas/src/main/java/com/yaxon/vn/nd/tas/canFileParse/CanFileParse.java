...
public class CanFileParse {
    ...
    public List<CanAnalyzeSignal> getAnalyzeSignalList(CanBusParamItem canBusParamItem, String modelType) {
        ...
        /**
         * 解析出来DBC文件中有用的数据(有效信息前后内容去掉)，按行存储
         */
        ...
        try {
            ...
            if (CanDBCFormat.OLD.name().equalsIgnoreCase(modelType)) {
                //优化:判断dbc文件没解析就解析,已经解析就直接读内存 modify by JianKang 2018-03-21
                if(null == CanConstants.t38Msg) {
                    CanConstants.t38Msg = getT38MsgComputer(msgDefAndValues);
                }
            } else if (CanDBCFormat.CURRENT.name().equalsIgnoreCase(modelType)) {
                if(null == CanConstants.t38Msg) {
                    CanConstants.t38Msg = getT38MsgComputerNew(msgDefAndValues);
                }
            }
            for (Map.Entry<CanMessageBean, List<CanSignalBean>> entries : CanConstants.t38Msg.entrySet()) {
