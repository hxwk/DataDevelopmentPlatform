package com.yaxon.vn.nd.ne.tas.handler;
...
import com.yaxon.vn.nd.ne.tas.net.tcp.TcpConnection;
import com.yaxon.vn.nd.ne.tas.net.tcp.TcpConnectionManager;
import com.yaxon.vn.nd.ne.tas.net.udp.UdpConnectionManager;
...
public abstract class BaseProtoHandler implements IProtocolHandler, MessageHandler {
    ...
    private InetSocketAddress vin2udpAddress;
    ...
    protected void sendCenterGeneralRes(ProtoMsg req, byte rc) throws TasException {
        ...
        vin2udpAddress = udpConnectionManager.getInetSocketAddressByVin(req.vin);

        if (vin2tcpConnection != null) {
            tcpChannel.sendMessage(msg);
        } else if (vin2udpAddress != null ) {
            msg.sender = vin2udpAddress;
    ...
    protected void sendMessage(ProtoMsg req) throws TasException {
        ...
        if (vin2tcpConnection != null ) {
            ...
            req.sender = vin2udpAddress;
            udpChannel.sendRequest(req);
        } else {
            logger.warn("终端未登录,vid:{} ", req.vin);
        }
    }

    protected void closeConnByVin(String vin) throws TasException {
