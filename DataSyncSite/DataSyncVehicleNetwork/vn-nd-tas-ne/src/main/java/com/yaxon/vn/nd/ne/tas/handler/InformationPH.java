...
import com.yaxon.vn.nd.ne.tas.exception.UnsupportedProtocolException;
import com.yaxon.vn.nd.ne.tas.net.proto.ProtoConstants;
import com.yaxon.vn.nd.ne.tas.net.proto.ProtoMsg;
import com.yaxon.vn.nd.tbp.si.Res_8001;
...
public class InformationPH extends BaseProtoHandler {
    ...
    /*public static void main(String[] args) {
        ...
        //03 fuel Cell
        String s = "1202021119260320202121222200024041001A0201110101110101";
    ...
    protected void doUpMsg(ProtoMsg upMsg) {
        //实时信息上报
        if (upMsg.commandSign == 0x02) {
            do_02(upMsg);
        }//补发信息上报
        else if (upMsg.commandSign == 0x03) {
            do_03(upMsg);
        } else {
     ...
     * @return
     */
    private void do_02(final ProtoMsg upMsg) {
        do_realDataOrAddData(upMsg);
    }
     ...
     * @return
     */
    private void do_03(final ProtoMsg upMsg) {
        do_realDataOrAddData(upMsg);
    }
     ...
     * @param upMsg
     */
    private void do_realDataOrAddData(final ProtoMsg upMsg) {
        ...
        while (msgBody.readableBytes() > 0) {
            ...
            switch (inforType){
                ...
                case NEStateConstant.ALARM://7.报警数据解析
                    ...
                    break;
                default:
                    logger.info("no match handler method");
            }
        }
        //发送tbp（终端业务处理模块）处理
        ListenableFuture<Message> f = tbp().call(req_02);
