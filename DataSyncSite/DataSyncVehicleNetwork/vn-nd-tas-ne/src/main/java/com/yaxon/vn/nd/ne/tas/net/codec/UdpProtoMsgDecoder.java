...
import com.yaxon.vn.nd.ne.tas.net.proto.*;
...
public class UdpProtoMsgDecoder extends MessageToMessageDecoder<DatagramPacket> {
    private static final Logger logger = LoggerFactory.getLogger(UdpProtoMsgDecoder.class);
    ...
    public UdpProtoMsgDecoder(UdpChannel udpChannel) {
        this.udpChannel = udpChannel;
        this.skipPackMergeProtos = udpChannel.getSkipPackMergeProtos();
        this.receiveTimeoutMillisPerPack = udpChannel.getConfig().getReceiveTimeoutMillisPerPack();
    ...
    @Override
    protected void decode(ChannelHandlerContext ctx, DatagramPacket msg, List<Object> msgs) throws Exception {
        try {
            ByteBuf packBuf = msg.content();
            ...
            if (header.splitFlag == 1 && !skipPackMergeProtos.contains(header.msgId)) {
                ...
                if (bb != null) {
                    header.packCount = 0;
                    header.packIndex = 0;
                    msgs.add(buildProtoMsg(header, bb,msg.recipient()));
                } else {
                ...
                packBuf.retain();
                msgs.add(buildProtoMsg(header, packBuf,msg.recipient()));
            }
        } catch (Exception e) {
    ...
    private ProtoMsg buildProtoMsg(ProtoPackHeader header, ByteBuf dataBuf, InetSocketAddress sender) {
