...
import com.yaxon.vn.nd.ne.tas.net.tcp.InvokeStubItem;
import com.yaxon.vn.nd.ne.tas.net.tcp.InvokeStubKey;
import com.yaxon.vn.nd.ne.tas.net.tcp.ProtocolDispatcher;
...
public class UdpChannel implements AccessChannel {
    ...
    public ListenableFuture<ProtoMsg> sendRequest(final ProtoMsg req, final short resId) {
        ...
        try {
            ...
            req.sn = getSn(req);
            req.sim = connectionManager.getSimByVid(req.vin);
    ...
    public void sendMessage(ProtoMsg msg) throws TasException {
        try {
            ...
            if(msg.sim =="" && msg.vin !=""){
                msg.sim = connectionManager.getSimByVid(msg.vin);
            }
            msg.sn = getSn(msg);
...
*/
    ...
    private boolean doUpGeneralRes(ProtoMsg msg) {
        ...
        InvokeStubItem item = invokeStubs.remove(new InvokeStubKey(msg.vin, msg.commandSign));
        if (item != null) {
            item.future.set(msg);
            return true;
        }
