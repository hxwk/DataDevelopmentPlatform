...
public class ProtoCommon {
  ...
  public static String getToken(String remote_filename, int ts, String secret_key) throws UnsupportedEncodingException, NoSuchAlgorithmException, MyException {
    byte[] bsFilename = remote_filename.getBytes(FastDFSEnviroment.g_charset);
    byte[] bsKey = secret_key.getBytes(FastDFSEnviroment.g_charset);
    byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(FastDFSEnviroment.g_charset);
