...
import com.dfssi.dataplatform.datasync.common.common.Base64;
...
public class StorageClient {
  ...
  /**
   * constructor using global settings in class FastDFSEnviroment
   */
  ...
  protected String[] do_upload_file(byte cmd, String group_name, String master_filename,
    ...
    try {
      ...
      if (file_ext_name != null && file_ext_name.length() > 0) {
        byte[] bs = file_ext_name.getBytes(FastDFSEnviroment.g_charset);
      ...
      if (bUploadSlave) {
        masterFilenameBytes = master_filename.getBytes(FastDFSEnviroment.g_charset);
      ...
      if (bUploadSlave) {
        ...
        byte[] bs = prefix_name.getBytes(FastDFSEnviroment.g_charset);
  ...
  protected int do_append_file(String group_name, String appender_filename,
    ...
    try {
      ...
      appenderFilenameBytes = appender_filename.getBytes(FastDFSEnviroment.g_charset);
  ...
  public NameValuePair[] get_metadata(String group_name, String remote_filename) throws IOException, MyException {
    ...
    try {
      ...
      return ProtoCommon.split_metadata(new String(pkgInfo.body, FastDFSEnviroment.g_charset));
    } catch (IOException ex) {
      if (!bNewConnection) {
        try {
          this.storageServer.close();
  ...
  public int set_metadata(String group_name, String remote_filename,
    ...
    try {
      ...
      if (meta_list == null) {
        ...
        meta_buff = ProtoCommon.pack_metadata(meta_list).getBytes(FastDFSEnviroment.g_charset);
      }

      filenameBytes = remote_filename.getBytes(FastDFSEnviroment.g_charset);
      sizeBytes = new byte[2 * ProtoCommon.FDFS_PROTO_PKG_LEN_SIZE];
      ...
      bs = group_name.getBytes(FastDFSEnviroment.g_charset);
