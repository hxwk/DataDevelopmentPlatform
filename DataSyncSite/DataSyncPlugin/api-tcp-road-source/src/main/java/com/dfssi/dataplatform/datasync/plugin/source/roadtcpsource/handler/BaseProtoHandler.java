...
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.common.*;
...
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.net.tcp.*;
...
public abstract class BaseProtoHandler implements IProtocolHandler {
    ...
    protected ListenableFuture<ProtoMsg> sendRequest(ProtoMsg req, short resId) {
        ...
        sim2tcpConnection = tcpConnectionManager.getConnectionBySim(req.sim);
        vid2tcpConnection = tcpConnectionManager.getConnectionByVid(req.vid);
    ...
    protected ListenableFuture<ProtoMsg> sendRequest(ProtoMsg req, short resId,String... redisKey) {
        AccessChannel tcpChannel = tcpChannel();
        sim2tcpConnection = tcpConnectionManager.getConnectionBySim(req.sim);
        vid2tcpConnection = tcpConnectionManager.getConnectionByVid(req.vid);
        if (vid2tcpConnection == null ){
            logger.debug("vid2tcpConnection为空");
        }else{
            logger.debug("根据vid2tcpConnection获取到链接");
        }

        if (sim2tcpConnection == null) {
            logger.debug("sim2tcpConnection为空");
        }else{
            logger.debug("根据sim2tcpConnection获取到链接");
        }
        ...
        if (vid2tcpConnection != null || sim2tcpConnection != null) {
            ...
            logger.warn("终端未登录,vid:{},sim:{} ,请检查车辆是否和平台建立连接", req.vid, req.sim);
            logger.error("下发指令异常,终端未登录，将结果存入redis，key: " + redisKey[0]);
            if(redisKey.length==2){
                jedis.hset(redisKey[0],redisKey[1] ,"notLogin");
            }else{
                jedis.set(redisKey[0], "notLogin");
            }
            jedis.close();
            return null;
        }
    ...
    protected void sendMessage(ProtoMsg msg) throws TcpException {
        AccessChannel tcpChannel = tcpChannel();
        sim2tcpConnection = tcpConnectionManager.getConnectionBySim(msg.sim);
        vid2tcpConnection = tcpConnectionManager.getConnectionByVid(msg.vid);
    ...
    /*public void processEvent(VnndResMsg res, String taskId, String msgid,String topic, ChannelProcessor channelProcessor){
        ...
        try {
            ...
            logger.info("发送event到channel成功 " + taskId + ", msgid = " + msgid,", event="+event);
        } catch (Exception e) {
            logger.error(null, e);
        }
    }

    public void processEvent(String json, String taskId, String msgid,String topic, ChannelProcessor channelProcessor){
        //key是event中header的Map的key
        logger.info("  开始封装event " + taskId + ", header的msgid = " + msgid+",key:"+Constants.KEY_HEADER);
        try {
            SimpleEvent event = new SimpleEvent();
            KafkaHeader header = new KafkaHeader();
            ...
            event.getHeaders().put(Constants.KEY_HEADER, JSONObject.toJSONString(header));
            channelProcessor.processEvent(event);
            logger.info("发送event到channel成功 " + taskId + ", msgid = " + msgid);
        } catch (Exception e) {
            logger.error(null, e);
        }
    }
        ...
        for (String vid : map.keySet()){
            if((currentTime-map.get(vid))/(60*1000)<10){
                ...
                if(!jedis.exists(vid)|| !jedis.get(Constants.GK_VEHICLE_STATE + vid).equals(flag)){
                    ...
                    //推送kafka
                    ProcessKafka.pushToKafka(json.toString(),"ROAD_VEHICLE_STATE_TOPIC",channelProcessor);
                }
    ...
    public void updateONStatus2Redis(JSONObject json, ChannelProcessor channelProcessor) {
        ...
        try {
            if(!jedis.exists(Constants.VEHILCE_ON_UPDATE + vid)|| !jedis.get(Constants.VEHILCE_ON_UPDATE + vid).equals(OnStatus)){
                ...
                //推送kafka
                ProcessKafka.pushToKafka(json.toString(),"VEHICLE_ON_STATE_TOPIC",channelProcessor);
            }
        } catch (Exception e) {
