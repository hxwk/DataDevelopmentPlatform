...
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.net.proto.SplitPackItem;
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.net.proto.SplitPackKey;
...
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.net.proto.*;
...
public class ProtoMsgDecoder extends MessageToMessageDecoder<ByteBuf> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf packBuf, List<Object> msgs) throws Exception {
        logger.trace("解码第四步:ProtoMsgDecoder的decode对进来的字节流进行解码成对象");
        try {
            ProtoPackHeader header = readProtoPackHeader(packBuf);
            logger.trace("headerSplitFlag:{},skipPackMergeProtos:{},msgId:{}",header.splitFlag,skipPackMergeProtos,CodecUtils.shortToHex(header.msgId));
            if (header.splitFlag == 1 && !skipPackMergeProtos.contains(header.msgId)) {
                //拼包处理
                logger.trace("开始进行拼包处理msgId:"+CodecUtils.shortToHex(header.msgId));
                ...
                logger.trace("不进行拼包处理msgId:"+CodecUtils.shortToHex(header.msgId));
                packBuf.retain();
    ...
    private void sendCenterGeneralRes(Channel channel, final String sim, short reqMsgId, short reqSn, byte retCode) {
        ProtoMsg msg = new ProtoMsg();
        msg.sim = sim;
