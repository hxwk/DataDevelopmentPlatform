...
public class CanFileParse {
     ...
     * 解析DBC配置文件到list
     */
    public CanFileParse(String dbcName) {
        init(dbcName);
    }

    private void init(String dbcName) {
        Map<String, Queue<String>> fileDataVersion = CanConstants.fileDataVersion;
        String filePath = localFilePath + File.separatorChar + dbcName;
        File dbcFilePath = new File(filePath);
        if (MapUtils.isNotEmpty(fileDataVersion)) {
            if (fileDataVersion.keySet().contains(dbcName)) {
                getValidT38Data = fileDataVersion.get(dbcName);
            } else {
                getValidT38Data = new CanDBCFileRead(dbcFilePath).readValidData();
                fileDataVersion.put(dbcName, getValidT38Data);
            }
        } else {
            getValidT38Data = new CanDBCFileRead(dbcFilePath).readValidData();
            fileDataVersion.put(dbcName, getValidT38Data);
    ...
    public List<CanAnalyzeSignal> getAnalyzeSignalList(CanBusParamItem canBusParamItem, String modelType) {
        ...
        try {
            ...
            if (CanDBCFormat.OLD.name().equalsIgnoreCase(modelType)) {
                ...
                //logger.warn("当前用current 格式文件解析方式解析的...");
