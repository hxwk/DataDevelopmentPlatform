...
/**
 * ftp我服务器上传文件和从ftp服务器下载文件
 */
public class FtpApache {
    private static FTPClient ftpClient = new FTPClient();
    private static String encoding = System.getProperty("file.encoding");
    private final static Logger logger = LoggerFactory.getLogger(FtpApache.class);
    /**
        ...
        try {
            ...
            for (FTPFile ff : fs) {
                if (ff.getName().equals(fileName)) {
                    ...
                    if(flag){
                        logger.info("retrieve file success.");
                    }else{
                        logger.warn("retrieve file failed.");
                    }
                    is.close();
                }
            }
    ...
    public void testDownFile() {
        try {
            boolean flag = downFile("172.16.1.180", 21, "ssi-ter",
                    "SSI-NanDou-ter", "/var/ftp/pub/ndypt/application/roadVehicle/upload/", "test.dbc", "G:/");
            System.out.println(flag);
        } catch (Exception e) {
            e.printStackTrace();
        }
