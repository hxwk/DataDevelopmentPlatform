...
import com.dfssi.dataplatform.datasync.plugin.source.roadtcpsource.util.BitParser;
...
public class PositionAndAlarmPH extends BaseProtoHandler {
    ...
    public void doUpMsg(ProtoMsg upMsg, String taskId, ChannelProcessor channelProcessor) {
        if (upMsg.msgId == 0x0200) {//位置信息汇报
            ...
            throw new UnsupportedProtocolException("PositionAndAlarmPH未知的上行请求消息：msgId=" + upMsg.msgId);
        }
    }
    ...
    public void doDnReq(Message dnReq, String taskId, ChannelProcessor channelProcessor) {
        if (dnReq instanceof Req_8201) {//位置信息查询
            ...
            throw new UnsupportedProtocolException("PositionAndAlarmPH未知的下行请求消息类型：" + dnReq.getClass().getName());
        }
    }
    ...
    private void do_0200(final ProtoMsg upMsg, String taskId, ChannelProcessor channelProcessor) {
        //logger.info("PositionAndAlarmPH开始处理0200协议报文");
        ...
        try {
            ...
            logger.info("解析sim:[{}]的0200上行消息并转发kafka:"+JSON.toJSONString(gps), upMsg.sim);
            ProcessKafka.processEvent(JSON.toJSONString(gps), taskId, q.id(), Constants.POSITIONINFORMATION_TOPIC, channelProcessor);

            //logger.info("[{}]接收到0200上行请求消息,不转发kafka:{}", upMsg.sim, q);
        ...
        try {
            ...
            for (int i = 0; (i < dataCount) && (msgData.readableBytes() > 28); i++) {
                ...
                if (dataLen > 0) {
                    ...
                    ProcessKafka.processEvent(JSON.toJSONString(addGpsVo), taskId, q.id(), Constants.POSITIONINFORMATIONS_TOPIC, channelProcessor);
                }
            }
        } catch (Exception e) {
            logger.warn("协议解析失败:" + upMsg, e);
