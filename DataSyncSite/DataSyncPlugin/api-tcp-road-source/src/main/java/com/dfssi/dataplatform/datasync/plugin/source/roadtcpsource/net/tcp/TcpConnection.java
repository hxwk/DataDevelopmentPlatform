...
public class TcpConnection extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        //这个时候没有获取到sim信息
        logger.info("[{}:{}]连接已建立,connId:{}", remoteHost,remotePort,connId);
        //flume采集连接状态
        //close();
    ...
    public void channelRead(ChannelHandlerContext ctx, Object protoMsg) throws Exception {
        ...
        try {
            logger.info("channelRead读取数据,msgid:" + CodecUtils.shortToHex(msg.msgId) + ",sim:" + msg.sim);
            ...
            if (state != CS_AUTHENCATED
                ...
                //如果报文发送过来，状态为未鉴权，且该报文的msgId不为终端注册、终端鉴权、终端注销、终端心跳，则抛出未鉴权异常
                logger.info("state:"+state+",msgId:"+msg.msgId );
                checkRepeatLink(msg);
            ...
            logger.debug("2.8TcpConnection的channelRead中终端回传数据此时vid:"+vid);
            msg.vid = vid;
            lastCommTime.set(System.currentTimeMillis());
...
//        }
    ...
    private void doTerminalRegisterRes(ProtoMsg msg) {
        logger.debug("[{}] 终端对平台进行注册,msg:{}", msg.sim,msg);
    ...
    private void doTerminalAuthRes(ProtoMsg msg) {
        logger.debug("[{}] 终端对平台进行鉴权,msg:{}", msg.sim,msg);
        ByteBuf dataBuf = msg.dataBuf;
        int readerIndex = dataBuf.readerIndex();
        short reqId = dataBuf.getShort(readerIndex + 2);
