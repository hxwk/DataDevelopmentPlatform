...
import com.dfssi.dataplatform.datasync.flume.agent.event.JSONEvent;
...
public class JSONHandler implements HTTPSourceHandler {
  ...
  public List<Event> getEvents(HttpServletRequest request) throws Exception {
    ...
    for (Event e : eventList) {
      ...
      header.put("key", JSONObject.toJSONString(e.getHeaders()));
      header.put(HTTPSourceConfigurationConstants.TASK_ID,taskId);
      e.setHeaders(header);
  ...
  private List<Event> getSimpleEvents(List<Event> events) {
    ...
    for (Event e:events) {
      Event newEvent = EventBuilder.withBody(e.getBody());
      e.getHeaders().put(HTTPSourceConfigurationConstants.TASK_ID,taskId);
