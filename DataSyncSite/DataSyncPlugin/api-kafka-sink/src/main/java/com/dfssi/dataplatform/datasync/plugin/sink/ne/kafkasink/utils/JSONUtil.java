package com.dfssi.dataplatform.datasync.plugin.sink.ne.kafkasink.utils;

/**
 * Created by HSF on 2017/12/6.
 */
...
public class JSONUtil {
     ...
     * <p />
     * <b>该方法转换发生错误时，不会抛出任何异常。若发生错误时，曾通对象返回 <code>"{}"</code>； 集合或数组对象返回 <code>"[]"</code>
     * </b>
     *
     * @param target 目标对象。
    ...
    /**
     * 将给定的目标对象转换成 {@code JSON} 格式的字符串。<b>此方法只用来转换普通的 {@code JavaBean} 对象。</b>
     * <ul>
    ...
    /**
     * 将给定的目标对象转换成 {@code JSON} 格式的字符串。<b>此方法通常用来转换使用泛型的对象。</b>
     * <ul>
     * <li>该方法只会转换标有 {@literal @Expose} 注解的字段；</li>
     * <li>该方法不会转换 {@code null} 值字段；</li>
     * <li>该方法会转换所有未标注或已标注 {@literal @Since} 的字段；</li>
    ...
    /**
     * 将给定的 {@code JSON} 字符串转换成指定的类型对象。<b>此方法通常用来转换普通的 {@code JavaBean} 对象。</b>
     *
     * @param <T> 要转换的目标类型。
     * @param json 给定的 {@code JSON} 字符串。
     * @param clazz 要转换的目标类。
