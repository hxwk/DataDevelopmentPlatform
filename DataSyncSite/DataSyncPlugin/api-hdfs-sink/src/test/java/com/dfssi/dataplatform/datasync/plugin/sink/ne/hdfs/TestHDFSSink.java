...
import com.dfssi.dataplatform.datasync.plugin.sink.hdfs.BucketWriter;
import com.dfssi.dataplatform.datasync.plugin.sink.hdfs.HDFSWriter;
import com.dfssi.dataplatform.datasync.plugin.sink.hdfs.SSIHDFSEventSink;
...
/**
 * Created by jian on 2017/12/6.
 */
public class TestHDFSSink {

    private SSIHDFSEventSink sink;
    ...
    private void dirCleanup() {
        ...
        try {
            ...
            logger.warn("IO Error in test cleanup", eIO);
        }
    }
     ...
     */
        testPath = "hdfs://172.16.1.210:8020/tmp/ssi_flume_t."
...
/*    @Test
    ...
    public void doTestTextBatchAppend(boolean useRawLocalFileSystem)
        ...
        for (i = 1; i <= (rollCount * 10) / batchSize; i++) {
            ...
            for (j = 1; j <= batchSize; j++) {
                ...
                eventDate.clear();
                /**
                 * generate events, including head and body
                 */
    ...
    public void doTestTextBatchAppend1(boolean useRawLocalFileSystem)
            throws Exception {
        logger.info("debug starting......");
        final long rollCount = 10;
        final long batchSize = 2;
        final String fileName = "FlumeData";
        String newPath = testPath + "/singleTextBucket";
     ...
     * context.get("hdfs.rollSize", String.class);
     * context.get("hdfs.rollCount", String.class);
     */
        Configurables.configure(sink, context);
    ...
    public void testBadSimpleAppend() throws InterruptedException,
        ...
        sink = new SSIHDFSEventSink(badWriterFactory);
        ...
        for (i = 1; i < numBatches; i++) {
            ...
            logger.info("Process events: " + sink.process());
        }
        logger.info("Process events to end of transaction max: " + sink.process());
        logger.info("Process events to injected fault: " + sink.process());
        logger.info("Process events remaining events: " + sink.process());
        sink.stop();
        verifyOutputSequenceFiles(fs, conf, dirPath.toUri().getPath(), fileName, bodies);
    ...
    private void verifyOutputSequenceFiles(FileSystem fs, Configuration conf, String dir,
            ...
            if (name.startsWith(prefix)) {
                ...
                while (reader.next(key, value)) {
                    ...
                    if (bodies.contains(body)) {
                        logger.debug("Found event body: {}", body);
                        bodies.remove(body);
                        found++;
                    }
                }
        ...
        if (!bodies.isEmpty()) {
            for (String body : bodies) {
                logger.error("Never found event body: {}", body);
            }
        }
    ...
    public void testCloseRemovesFromSFWriters()
        ...
        logger.info("execute sink to process the events: " + sink.process());
        ...
        logger.info("Total number of bucket writers opened: {}",
    ...
    private void testRetryRename(boolean closeSucceed)
        ...
        for (BucketWriter writer : writers) {
            logger.info("Rename tries = " + writer.renameTries.get());
    ...
    /**
     * get configuration DataDestination class, to get info
     * @throws IOException
     * @throws EventDeliveryException
     */
    /*@Test
    public void testSimpleAppendText() throws IOException, EventDeliveryException{
        logger.debug("Starting execute text files sink to hdfs ...");
        final String fileName = "dfssi";
        TaskDataDestination dataDestination = new TaskDataDestination();
        Map<String,String> config = dataDestination.getConfig();
        final long rollCount = 5;
        final long batchSize = 2;
        final int numBatches = 4;
        String newPath = testPath + "/singleBucket";
