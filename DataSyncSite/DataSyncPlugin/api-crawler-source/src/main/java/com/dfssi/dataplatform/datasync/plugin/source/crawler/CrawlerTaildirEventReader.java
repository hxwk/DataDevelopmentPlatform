...
/**
 * flume , crawler taildir event reader
 * @author jianKang
 * @date 2017/11/24
 */
public class CrawlerTaildirEventReader {
    private static final Logger logger = LoggerFactory.getLogger(CrawlerTaildirEventReader.class);

    private final List<CrawlerTaildirMatcher> taildirCache;
    private final Table<String, String, String> headerTable;

    private CrawlerTailFile currentFile = null;
    private Map<Long, CrawlerTailFile> tailFiles = Maps.newHashMap();
    ...
    /**
     * Create a CrawlerTaildirEventReader to watch the given directory.
     */
    private CrawlerTaildirEventReader(Map<String, String> filePaths,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, metaDir={}",
                    new Object[] { CrawlerTaildirEventReader.class.getSimpleName(), filePaths });
        }

        List<CrawlerTaildirMatcher> taildirCache = Lists.newArrayList();
        for (Map.Entry<String, String> e : filePaths.entrySet()) {
            taildirCache.add(new CrawlerTaildirMatcher(e.getKey(), e.getValue(), cachePatternMatching));
        }
    ...
    public void loadPositionFile(String filePath) {
        ...
        try {
            ...
            while (jr.hasNext()) {
                ...
                CrawlerTailFile tf = tailFiles.get(inode);
    ...
    public Map<Long, CrawlerTailFile> getTailFiles() {
        return tailFiles;
    }

    public void setCurrentFile(CrawlerTailFile currentFile) {
    ...
    @VisibleForTesting
    public List<Event> readEvents(CrawlerTailFile tf, int numEvents) throws IOException {
        setCurrentFile(tf);
    ...
    public void close() throws IOException {
        for (CrawlerTailFile tf : tailFiles.values()) {
            if (tf.getRaf() != null) {
                tf.getRaf().close();
            }
        }
    ...
    public List<Long> updateTailFiles(boolean skipToEnd) throws IOException {
        ...
        for (CrawlerTaildirMatcher taildir : taildirCache) {
            Map<String, String> headers = headerTable.row(taildir.getFileGroup());

            for (File f : taildir.getMatchingFiles()) {
                long inode = getInode(f);
                CrawlerTailFile tf = tailFiles.get(inode);
                if (tf == null || !StringUtils.equals(tf.getPath(),f.getAbsolutePath())) {
    ...
    private CrawlerTailFile openFile(File file, Map<String, String> headers, long inode, long pos) {
        try {
            logger.info("Opening file: " + file + ", inode: " + inode + ", pos: " + pos);
            return new CrawlerTailFile(file, headers, inode, pos);
        } catch (IOException e) {
    ...
    /**
     * Special builder class for CrawlerTaildirEventReader
     */
    public static class Builder {
        ...
        private Boolean annotateFileName =
                CrawlerSourceConfigurationConstants.DEFAULT_FILE_HEADER;
        private String fileNameHeader =
                CrawlerSourceConfigurationConstants.DEFAULT_FILENAME_HEADER_KEY;
        ...
        public CrawlerTaildirEventReader build() throws IOException {
            return new CrawlerTaildirEventReader(filePaths, headerTable, positionFilePath, skipToEnd,
