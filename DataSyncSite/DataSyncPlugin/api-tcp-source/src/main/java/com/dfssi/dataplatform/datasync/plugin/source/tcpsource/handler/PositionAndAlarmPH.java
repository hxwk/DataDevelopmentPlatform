...
//        int count = upMsg.dataBuf.readableBytes();
//        byte[] out = new byte[count];
//        upMsg.dataBuf.readBytes(out);
//        logger.info("进入0200查看一下数据"+hex.encode(out));
//        upMsg.dataBuf.resetReaderIndex();
        //解析上行请求协议
        try {
            ...
            while (reqBuf.readableBytes() > 0) {
                ...
                } else if (extraInfoId == 0xEF) { //abs
//                    reqBuf.skipBytes(1);
//                    gps.setCollTime(Long.toHexString(reqBuf.readLong()));
//                    gps.setSensorSpeed(reqBuf.readShort());
//                    gps.setWheelSpeed(reqBuf.readShort());
//                    int tmp = reqBuf.readInt();
//                    gps.setBraking(tmp & 0x01);
//                    gps.setGear((tmp>>1) & 0x000F);
//                    gps.setBrakeStatus((tmp>>5) & 0x01);
//                    gps.setAbsStatus((tmp>>6) & 0x01);
//                    gps.setExtraInfoItems(extraInfoItems);
//                    gps.setMsgId("0200abs");
//                    gps.setReceiveMsgTime(System.currentTimeMillis());
//                    processEvent(JSON.toJSONString(gps), taskId, q.id(), "POSITIONINFORMATION_0200_ABS_TOPIC", channelProcessor);
                }else if (extraInfoId == 0xE1) { //abs高精度
//                    reqBuf.skipBytes(1);
//                    gps.setHighPrecisionFlag(reqBuf.readByte());
//                    gps.setHighPrecisionLat(reqBuf.readLong()+"");
//                    gps.setHighPrecisionLon(reqBuf.readLong()+"");
//                    gps.setDirection(reqBuf.readInt());
//                    gps.setMsgId("0200abs");
//                    gps.setReceiveMsgTime(System.currentTimeMillis());
//                    logger.info("看看abs接入的数据："+gps);
//                    processEvent(JSON.toJSONString(gps), taskId, q.id(), "POSITIONINFORMATION_0200_ABS_TOPIC", channelProcessor);

                    reqBuf.skipBytes(1);
                    gps.setHighPrecisionFlag(reqBuf.readByte());
                    byte groupNum = reqBuf.readByte();
                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    for(int i=0;i< groupNum;i++){
                        gps.setHighPrecisionLat(reqBuf.readLong()+"");
                        gps.setHighPrecisionLon(reqBuf.readLong()+"");
                        gps.setDirection(reqBuf.readInt());
                        gps.setAbsSpeed(reqBuf.readInt());
                        String abstime = reqBuf.readInt()+"";
                        String substring = abstime.substring(0, abstime.length() - 3);
                        int abstimeTmp=Integer.parseInt(substring);
                        String changeTime=abstimeTmp/3600+":"+(abstimeTmp%3600)/60+":"+(abstimeTmp%3600)%60;
                        String format = df.format(new Date());
                        String time = format.substring(0, 10) +" "+ changeTime;
                        Date date = df.parse(time);
                        long ts = date.getTime()+Integer.parseInt(abstime.substring(abstime.length() - 3));
                        gps.setAbsTime(ts+"");
                        gps.setMsgId("0200abs");
                        gps.setReceiveMsgTime(System.currentTimeMillis());
                        logger.info("看看abs接入的数据："+gps);
