...
//		exe.submit(status);
	...
	@Override
	public void stop() {
		logger.info("--------------测试TCPSource的stop方法");
		try {
			if (tcpChannel != null) {
				tcpChannel.stop();
				tcpChannel = null;
	...
	public VnndResMsg receive(Message req) {
		...
		logger.info("com.dfssi.dataplatform.datasync.plugin.source.tcpsource.server.TCPSource接收消息 req =" + req);

		BaseProtoHandler handler = HandlersManger.getDownHandlers(req.id());
		VnndResMsg jtsresmsg=null;
		if (handler == null) {
			logger.warn("未找到消息处理器:msgId=" + req.id());
		} else {
			try {
