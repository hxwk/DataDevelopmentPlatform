package com.dfssi.dataplatform.datasync.flume.agent.lifecycle;
...
public class LifecycleSupervisor implements LifecycleAware {
  ...
  public synchronized void stop() {
    ...
    logger.info("Stopping lifecycle supervisor 线程号:{}", Thread.currentThread()
        .getId());

    if (monitorService != null) {
      //logger.info("模拟monitorService.shutdown，测试netty连接是否关闭");
  ...
  public synchronized void unsupervise(LifecycleAware lifecycleAware) {
    ...
    logger.info("Unsupervising service:{}", lifecycleAware);
    ...
    synchronized (lifecycleAware) {
      ...
      lifecycleAware.stop();
      logger.info("停止component: {}成功", lifecycleAware);
    }
