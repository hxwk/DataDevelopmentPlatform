...
public class EsClient{
    ...
    private HashMultimap<String, Integer> hostAndPort;
    ...
    private void readConf(EsBaseConf baseConf, Map<String, String> conf) {
        ...
        this.hostAndPort = HashMultimap.create();

        String item = String.format("es.%s.client.servers", clientId);
        String  itemValue = conf.get(item);
        if(itemValue == null) itemValue = baseConf.getConfigItemValue(item, "localhost:9300");
        ...
        try {
            ...
            sb.append("\n\t").append("servers = ").append(itemValue);
        } catch (NumberFormatException e) {
            logger.error(String.format("解析%s失败。", item), e);
        }
    ...
    private void buildClient() {
        try {
            ...
            for(Map.Entry<String, Integer> hp : hostAndPort.entries()) {
                client.addTransportAddress(new TransportAddress(InetAddress.getByName(hp.getKey()), hp.getValue()));
            }
        } catch (Exception e) {
