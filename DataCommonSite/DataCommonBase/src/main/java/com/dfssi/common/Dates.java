...
public class Dates {
	...
	public static long getFiveEdge(long time){
		...
		try {
			...
			long gap = time - parse.getTime();

			long fives = gap / (5 * 60 * 1000);

			return parse.getTime() + fives * 5 * 60 * 1000;
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static long getTenEdge(long time){
		...
		try {
			...
			long tens = gap / (10 * 60 * 1000);

			return parse.getTime() + tens * 10 * 60 * 1000;
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		long fiveEdge = getFiveEdge(System.currentTimeMillis());
		System.out.println(nowStr(new Date(fiveEdge)));
	}
