package com.dfssi.common.file;

import com.dfssi.common.net.URLUtils;
...
public class Downloader extends Observable {
	...
	public Downloader(String url, String savePath) throws Exception {
		//超时一小时
		this(url, savePath, 1000 * 60*5,50);
	}

	public Downloader(String url, String savePath, long timeout,int maxThread) throws Exception {
		boolean exists = URLUtils.exists(url);
		if(!exists){
			throw new IllegalArgumentException(String.format("文件地址：%s 不存在。", url));
		}else {
	...
	private Timer timer() {
		...
		timer.schedule(new TimerTask() {
			@Override
			public void run() {
				log.info(String.format("已下载-->%s -> %s", (int)Math.ceil(((double) downloaded) / size * 100) + "%", url));
				//如果上一次的下载大小与当前的一样就退出
	...
	public void end(CountDownLatch countDownLatch, Timer timer){
		try {
			//超过指定时间就直接结束
