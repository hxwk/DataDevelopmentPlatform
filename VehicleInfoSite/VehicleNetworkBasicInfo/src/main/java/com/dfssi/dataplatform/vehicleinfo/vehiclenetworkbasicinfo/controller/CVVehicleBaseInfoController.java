 ...
 */
@Controller
@RequestMapping(value = "/cvvehicleBaseInfo")
@Api(value = "vehicleBaseInfoController", description = "燃油车车辆基础信息接入")
public class CVVehicleBaseInfoController {

    @Autowired
    private ICVVehicleBaseInfoService cvvehicleService;
    ...
    public ResponseObj findVehicleBaseInfo(@RequestBody String vehicle) {
        ...
        if(validParam) {
            ...
            if (StringUtils.isEmpty(msg)) {
                int count = cvvehicleService.findVehicleCount(vehicleDTO);
                List<CVVehicleDTO> vehicleList = new ArrayList<>();
                if (count > 0) {
                    vehicleList = cvvehicleService.findVehicleBaseInfo(vehicleDTO, null);
                }
    ...
    public ResponseObj batchSaveVehicleBaseInfo(@RequestBody String vehicles) {
        ...
        if(validParam) {
            ...
            for (int i = 0; i < vehicleList.size(); i++) {
                ...
                String saveResult = cvvehicleService.saveVehicleBaseInfo(vehicle);//保存数据到geode
    ...
    public ResponseObj batchDeleteVehicleBaseInfo(@RequestBody String vids) {
        ...
        if(validParam) {
            ...
            for (int k = 0; k < vehicleList.size(); k++) {
                ...
                String delResult = cvvehicleService.deleteVehicleBaseInfo(vehicleDTO);
...
//        msg=msg+validateData(vehicleDTO);
        if (StringUtils.isNotEmpty(msg)) {
            ...
            String saveResult = cvvehicleService.saveVehicleBaseInfo(vehicleDTO);//保存数据到geode
    ...
    public ResponseObj findVehicleBaseInfoByVidList(@RequestBody String vids) {
        ...
        if(validParam) {
            List<CVVehicleDTO> list = cvvehicleService.findVehicleBaseInfoByVidList(vehicleList);
            HashMap map = new HashMap();
