...
public class RedisPoolManager {
    ...
    private JedisPool getInstance() {
        ...
        if (pool == null) {
            ...
            logger.info("REDIS_SERVER:{},REDIS_PORT:{},PASSWORD:{},DATABASE:{} ", REDIS_SERVER, REDIS_PORT, PASSWORD, DATABASE);

            pool = new JedisPool(config, REDIS_SERVER, REDIS_PORT, 100000, PASSWORD, DATABASE);
        }
        return pool;
    }

    //从默认库0读写
    public JedisPool getInstanceDb0(){
        if( null == pool){
            JedisPoolConfig config = new JedisPoolConfig();
            config.setMaxTotal(MAX_TOTAL);
            config.setMaxIdle(MAX_IDLE);
