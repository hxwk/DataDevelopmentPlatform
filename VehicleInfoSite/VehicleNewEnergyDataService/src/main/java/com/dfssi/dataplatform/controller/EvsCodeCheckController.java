...
@Api(tags = {"新能源平台可充电储能系统编码长度检测"})
public class EvsCodeCheckController {
    private final Logger logger = LoggerFactory.getLogger(EvsCodeCheckController.class);

    @Autowired
    private EvsCodeCheckService evsCodeCheckService;


    @ApiOperation(value = "新能源平台可充电储能系统编码长度检测查询")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "vins", value = "vin数组"),
            @ApiImplicitParam(name = "startTime", value = "开始时间"),
            @ApiImplicitParam(name = "endTime", value = "结束时间")
    })
    @RequestMapping(value = "/findCodeCheckInfo", method = RequestMethod.POST)
    @ResponseBody
    @LogAudit
    public Object findCodeCheckInfo(@RequestBody String vehicles){
        ...
        if(validParam) {
            try {
                Map<String, Object> data = evsCodeCheckService.findCodeCheckInfo(vinList, startTime, endTime);
