 ...
 */
@Controller
@RequestMapping(value = "/vehicleMileage")
@Api(value = "VehicleMileageController", description = "车辆里程油耗信息")
public class VehicleMileageController {

    @Autowired
    private IVehicleMileageService iVehicleMileageService;
     ...
     * @return
     */
    @RequestMapping(value="/findMileageInfo",method= RequestMethod.POST)
    @ResponseBody
    @ApiOperation(value = "查询车辆里程油耗信息")
    @LogAudit
    public ResponseObj findMileageInfo(@RequestBody String vids){
        ...
        if(validParam) {
            ...
            List<VehicleMileageDTO> list=iVehicleMileageService.findMileageInfo(vidList);
