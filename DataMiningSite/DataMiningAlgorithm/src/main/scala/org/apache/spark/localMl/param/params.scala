...
package org.apache.spark.localMl.param
...
import org.apache.spark.localMl.linalg.JsonVectorConverter
...
class Param[T](val parent: String, val name: String, val doc: String, val isValid: T => Boolean)
   ...
   * columns should be implemented in [[org.apache.spark.localMl.PipelineStage.transformSchema()]].
   *
 ...
 */
@Since("1.2.0")
final class ParamMap private[localMl] (private val map: mutable.Map[Param[Any], Any])
  extends Serializable {
  ...
  private[localMl] def put(paramPairs: JList[ParamPair[_]]): this.type = {
  ...
  private[localMl] def toList: java.util.List[ParamPair[_]] = {
    this.toSeq.asJava
  }
