package org.apache.spark.localMl.knn

import breeze.linalg.{DenseVector, Vector => BV}
import breeze.stats._
import org.apache.spark.broadcast.Broadcast
import org.apache.spark.localMl.knn.KNN.{KNNPartitioner, RowWithVector, VectorWithNorm}
...
import org.apache.spark.localMl.classification.KNNClassificationModel
import org.apache.spark.localMl.regression.KNNRegressionModel
import org.apache.spark.localMllib.knn.KNNUtils
import org.apache.spark.localMllib.linalg.{Vector, VectorUDT, Vectors}
import org.apache.spark.localMllib.rdd.MLPairRDDFunctions._
...
private[localMl] trait KNNModelParams extends Params with HasFeaturesCol with HasInputCols {
  /**
  ...
  private[localMl] def transform(data: RDD[Vector], topTree: Broadcast[Tree], subTrees: RDD[Tree]): RDD[(Long, Array[Row])] = {
  ...
  private[localMl] def transform(dataset: DataFrame, topTree: Broadcast[Tree], subTrees: RDD[Tree]): RDD[(Long, Array[Row])] = {
    transform(dataset.select($(featuresCol)).rdd.map(_.getAs[Vector](0)), topTree, subTrees)
  }
}

private[localMl] trait KNNParams extends KNNModelParams with HasSeed {
  /**
...
/**
  ...
  *
  */
class KNNModel private[localMl](
...
class KNN(override val uid: String) extends Estimator[KNNModel] with KNNParams {
  ...
  override def fit(dataset: DataFrame): KNNModel = {
    ...
    val repartitioned = new ShuffledRDD[RowWithVector, Null, Null](data.map(v => (v, null)), part).keys
      ...
      //balanceThreshold
    println("balanceThreshold->"+$(balanceThreshold)+"  bufferSize->"+$(bufferSize))
    val tau =
...
object KNN extends Logging {
  ...
  def estimateTau(data: RDD[RowWithVector], sampleSize: Array[Int], seed: Long): Double = {
    ...
    val ymean = yMeanVariance.mean
      ...
      //add println
      val mathUp = mean(x:*y)-xmean*ymean
      val mathDown = math.sqrt((mean(x :* x) - xmean * xmean) * (mean(y :* y) - ymean * ymean))
      println("mathUp->"+mathUp+"   mathDown->"+mathDown)
  ...
  private[localMl] def searchIndices(v: VectorWithNorm, tree: Tree, tau: Double, acc: Int = 0): Seq[Int] = {
    tree match {
      case node: MetricTree =>
