...
import org.apache.spark.localMllib.linalg.VectorUDT
import org.apache.spark.sql.types._

/**
 * Represents a parsed R formula.
 */
private[localMl] case class ParsedRFormula(label: ColumnRef, terms: Seq[Term]) {
  /**
 ...
 */
private[localMl] case class ResolvedRFormula(
 ...
 */
private[localMl] sealed trait Term

/** A term that may be part of an interaction, e.g. 'x' in 'x:y' */
private[localMl] sealed trait InteractableTerm extends Term

/* R formula reference to all available columns, e.g. "." in a formula */
private[localMl] case object Dot extends InteractableTerm

/* R formula reference to a column, e.g. "+ Species" in a formula */
private[localMl] case class ColumnRef(value: String) extends InteractableTerm

/* R formula interaction of several columns, e.g. "Sepal_Length:Species" in a formula */
private[localMl] case class ColumnInteraction(terms: Seq[InteractableTerm]) extends Term

/* R formula intercept toggle, e.g. "+ 0" in a formula */
private[localMl] case class Intercept(enabled: Boolean) extends Term

/* R formula deletion of a variable, e.g. "- Species" in a formula */
private[localMl] case class Deletion(term: Term) extends Term

/**
 * Limited implementation of R formula parsing. Currently supports: '~', '+', '-', '.', ':'.
 */
private[localMl] object RFormulaParser extends RegexParsers {
