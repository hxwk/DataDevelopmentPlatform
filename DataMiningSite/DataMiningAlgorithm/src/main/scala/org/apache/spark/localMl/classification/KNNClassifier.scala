...
import org.apache.spark.localMl.knn._
import org.apache.spark.localMl.param.ParamMap
import org.apache.spark.localMl.param.shared.HasWeightCol
import org.apache.spark.localMl.util.{Identifiable, SchemaUtils}
...
with KNNParams with HasWeightCol with Logging {
  ...
  /**
    ...
    */
  override def extractLabeledPoints(dataset: DataFrame): RDD[LabeledPoint] = {
    dataset.select($(labelCol), $(featuresCol))
            .map { case Row(label: Double, features: Vector) => LabeledPoint(label, features) }
  }
}

class KNNClassificationModel private[localMl](
