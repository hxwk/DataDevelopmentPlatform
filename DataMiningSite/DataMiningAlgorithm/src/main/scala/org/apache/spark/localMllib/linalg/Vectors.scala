...
package org.apache.spark.localMllib.linalg
...
import org.apache.spark.localMllib.util.NumericParser
...
/**
 * Factory methods for [[org.apache.spark.localMllib.linalg.Vector]].
...
object Vectors {
  ...
  private[localMllib] def parseNumeric(any: Any): Vector = {
    any match {
   ...
   */
  private[localMllib] def sqdist(v1: SparseVector, v2: DenseVector): Double = {
    var kv1 = 0
    var kv2 = 0
...
class DenseVector @Since("1.0.0") (
  ...
  private[spark] override def asBreeze = toBreeze
...
object SparseVector {
  ...
  /**
    ...
    */
  @Since("2.0.0")
  def fromML(v: SparseVector): SparseVector = {
...
/**
  * Implicit methods available in Scala for converting [[org.apache.spark.localMllib.linalg.Vector]] to
  * [[org.apache.spark.localMllib.linalg.Vector]] and vice versa.

private[spark] object VectorImplicits {

  implicit def mllibVectorToMLVector(v: Vector): Vector = v.asML

  implicit def mllibDenseVectorToMLDenseVector(v: DenseVector): DenseVector = v.asML

  implicit def mllibSparseVectorToMLSparseVector(v: SparseVector): SparseVector = v.asML

  implicit def mlVectorToMLlibVector(v: Vector): Vector = Vectors.fromML(v)

  implicit def mlDenseVectorToMLlibDenseVector(v: DenseVector): DenseVector =
    Vectors.fromML(v).asInstanceOf[DenseVector]

  implicit def mlSparseVectorToMLlibSparseVector(v: SparseVector): SparseVector =
