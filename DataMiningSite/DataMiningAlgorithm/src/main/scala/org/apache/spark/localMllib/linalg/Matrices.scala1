...
sealed trait Matrix extends Serializable {
  ...
  private[localMllib] def toBreeze: BM[Double]

  private[spark] def asBreeze: BM[Double]
  ...
  private[localMllib] def index(i: Int, j: Int): Int

  /** Update element at (i, j) */
  private[localMllib] def update(i: Int, j: Int, v: Double): Unit
  ...
  private[localMllib] def update(f: Double => Double): Matrix
...
class DenseMatrix @Since("1.3.0") (
  ...
  override private[spark] def asBreeze = toBreeze

  private[spark] def apply(i: Int): Double = values(i)
  ...
  private[spark] def update(i: Int, j: Int, v: Double): Unit = {
    values(index(i, j)) = v
  }
  ...
  private[spark] def update(f: Double => Double): DenseMatrix = {
...
/**
 * Factory methods for [[org.apache.spark.localMllib.linalg.DenseMatrix]].
 */
@Since("1.3.0")
object DenseMatrix {
  ...
  /**
    ...
    */
  @Since("2.0.0")
  def fromML(m: DenseMatrix): DenseMatrix = {
...
class SparseMatrix @Since("1.3.0") (
  ...
  private[spark] def toBreeze: BM[Double] = {
     if (!isTransposed) {
  ...
  private[spark] def index(i: Int, j: Int): Int = {
    require(i >= 0 && i < numRows, s"Expected 0 <= i < $numRows, got i = $i.")
    require(j >= 0 && j < numCols, s"Expected 0 <= j < $numCols, got j = $j.")
    if (!isTransposed) {
  ...
  private[spark] def update(f: Double => Double): SparseMatrix = {
    val len = values.length
    var i = 0
    while (i < len) {
      values(i) = f(values(i))
...
/**
 * Factory methods for [[org.apache.spark.localMllib.linalg.SparseMatrix]].
 */
@Since("1.3.0")
object SparseMatrix {
...
/**
 * Factory methods for [[org.apache.spark.localMllib.linalg.Matrix]].
 */
@Since("1.0.0")
object Matrices {
   ...
   */
  private[spark] def fromBreeze(breeze: BM[Double]): Matrix = {
    breeze match {
      case dm: BDM[Double] =>
