...
package org.apache.spark.localMllib.util
...
import org.apache.spark.localMllib.linalg.{MatrixUDT => MLMatrixUDT, VectorUDT => MLVectorUDT}
import org.apache.spark.localMllib.regression.LabeledPoint
import org.apache.spark.localMllib.linalg.{DenseVector, SparseVector, Vector, Vectors}
import org.apache.spark.localMllib.linalg._
import org.apache.spark.localMllib.linalg.BLAS.dot
...
object MLUtils extends Logging {
  ...
  private[localMllib] lazy val EPSILON = {
    var eps = 1.0
   ...
   * This method parses each line into a [[org.apache.spark.localMllib.regression.LabeledPoint]],
   ...
   *
   * @see [[org.apache.spark.localMllib.util.MLUtils#loadLibSVMFile]]
   */
  @Since("1.0.0")
   ...
   *            [[org.apache.spark.localMllib.util.MLUtils#loadLabeledPoints]] for loading.
   */
  @Since("1.0.0")
  ...
  /**
    * Converts vector columns in an input Dataset from the [[org.apache.spark.localMllib.linalg.Vector]]
    * type to the new [[org.apache.spark.localMllib.linalg.Vector]] type under the `spark.ml` package.
  ...
  @Since("2.0.0")
  @varargs
  def convertVectorColumnsToML(dataset: DataFrame, cols: String*): DataFrame = {
  ...
  /**
    * Converts vector columns in an input Dataset to the [[org.apache.spark.localMllib.linalg.Vector]]
    * type from the new [[org.apache.spark.localMl.linalg.Vector]] type under the `spark.ml` package.
    * @param dataset input dataset
  ...
  @Since("2.0.0")
  @varargs
  def convertVectorColumnsFromML(dataset: DataFrame, cols: String*): DataFrame = {
    val schema = dataset.schema
    val colSet = if (cols.nonEmpty) {
      cols.flatMap { c =>
        val dataType = schema(c).dataType
  ...
  /**
    * Converts Matrix columns in an input Dataset from the [[org.apache.spark.localMllib.linalg.Matrix]]
    * type to the new [[org.apache.spark.localMl.linalg.Matrix]] type under the `spark.ml` package.
    * @param dataset input dataset
  ...
  @Since("2.0.0")
  @varargs
  def convertMatrixColumnsToML(dataset: DataFrame, cols: String*): DataFrame = {
    val schema = dataset.schema
    val colSet = if (cols.nonEmpty) {
      cols.flatMap { c =>
        val dataType = schema(c).dataType
  ...
  /**
    * Converts matrix columns in an input Dataset to the [[org.apache.spark.localMllib.linalg.Matrix]]
    * type from the new [[org.apache.spark.localMl.linalg.Matrix]] type under the `spark.ml` package.
    * @param dataset input dataset
  ...
  @Since("2.0.0")
  @varargs
  def convertMatrixColumnsFromML(dataset: DataFrame, cols: String*): DataFrame = {
    val schema = dataset.schema
    val colSet = if (cols.nonEmpty) {
      cols.flatMap { c =>
        val dataType = schema(c).dataType
   ...
   */
  private[localMllib] def fastSquaredDistance(
      v1: Vector,
      norm1: Double,
      v2: Vector,
      norm2: Double,
