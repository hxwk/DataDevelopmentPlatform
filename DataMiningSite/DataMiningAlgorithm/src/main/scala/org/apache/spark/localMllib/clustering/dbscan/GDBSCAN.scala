package org.apache.spark.localMllib.clustering.dbscan
  ...
  def cluster(data: DenseMatrix[T]): Seq[Cluster[T]] = {
    ...
    points.collect {
      case point @ Point(row) if !(visited contains point) =>
        val neighbours = getNeighbours(point, points.filterNot(x => (x.row == point.row || visited.contains(x))))
...
object GDBSCAN {
  ...
  case class Point[T](row: Int)(val value: DenseVector[T]) {
    override def toString() = s"${value.valueAt(0)},${value.valueAt(1)}"//s"[$row]: $value"
  }
