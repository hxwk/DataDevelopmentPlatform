...
import org.apache.spark.localMllib.tree.configuration.{FeatureType => OldFeatureType}
import org.apache.spark.localMllib.tree.model.{Split => OldSplit}
...
sealed trait Split extends Serializable {
   ...
   */
  private[localMl] def shouldGoLeft(features: Vector): Boolean
 ...
 */
class CategoricalSplit private[localMl] (
  ...
  override private[localMl] def shouldGoLeft(features: Vector): Boolean = {
    if (isLeft) {
 ...
 */
class ContinuousSplit private[localMl] (override val featureIndex: Int, val threshold: Double)
  extends Split {
