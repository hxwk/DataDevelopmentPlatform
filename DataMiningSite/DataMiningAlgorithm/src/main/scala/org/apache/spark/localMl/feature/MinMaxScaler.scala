...
import org.apache.spark.localMllib.linalg.{Vector, Vectors, VectorUDT}
import org.apache.spark.localMl.param.{DoubleParam, ParamMap, Params}
import org.apache.spark.localMl.param.shared.{HasInputCol, HasOutputCol}
import org.apache.spark.localMl.util._
import org.apache.spark.localMllib.stat.Statistics
...
class MinMaxScaler @Since("1.5.0") (@Since("1.5.0") override val uid: String)
  ...
  override def fit(dataset: DataFrame): MinMaxScalerModel = {
    ...
    val input: RDD[Vector] = dataset.select($(inputCol)).rdd
      .map {
 ...
 */
@Since("1.5.0")
class MinMaxScalerModel private[localMl] (
...
object MinMaxScalerModel extends MLReadable[MinMaxScalerModel] {
  ...
  private class MinMaxScalerModelReader extends MLReader[MinMaxScalerModel] {
    ...
    override def load(path: String): MinMaxScalerModel = {
      ...
      val Row(originalMin: Vector, originalMax: Vector) = data
