...
import org.apache.spark.localMllib.stat.{MultivariateOnlineSummarizer, MultivariateStatisticalSummary}
...
class RowMatrix @Since("1.0.0") (
   ...
   */
  private[localMllib] def multiplyGramianMatrixBy(v: BDV[Double]): BDV[Double] = {
    val n = numCols().toInt
   ...
   * @return SingularValueDecomposition(U, s, V). U = null if computeU = false.
   */
  private[localMllib] def computeSVD(
      k: Int,
      computeU: Boolean,
      rCond: Double,
      maxIter: Int,
   ...
   * @return a [[org.apache.spark.localMllib.linalg.distributed.RowMatrix]] representing the product,
   ...
   *         between columns of this matrix.
   */
  private[localMllib] def columnSimilaritiesDIMSUM(
