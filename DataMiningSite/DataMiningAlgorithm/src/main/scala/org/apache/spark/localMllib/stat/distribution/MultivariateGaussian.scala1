...
package org.apache.spark.localMllib.stat.distribution

import breeze.linalg.{DenseVector => DBV, DenseMatrix => DBM, diag, max, eigSym, Vector => BV}

import org.apache.spark.annotation.{DeveloperApi, Since}
import org.apache.spark.localMllib.linalg.{Vectors, Vector, Matrices, Matrix}
...
class MultivariateGaussian @Since("1.3.0") (
  ...
  /**
   * private[localMllib] constructor
   *
   * @param mu The mean vector of the distribution
   * @param sigma The covariance matrix of the distribution
   */
  private[localMllib] def this(mu: DBV[Double], sigma: DBM[Double]) = {
  ...
  /** Returns density of this multivariate Gaussian at given point, x */
  private[localMllib] def pdf(x: BV[Double]): Double = {
    math.exp(logpdf(x))
  }

  /** Returns the log-density of this multivariate Gaussian at given point, x */
  private[localMllib] def logpdf(x: BV[Double]): Double = {
