...
package org.apache.spark.localMl.stat

import org.apache.spark.annotation.{Experimental, Since}
import org.apache.spark.localMl.linalg.SQLDataTypes
import org.apache.spark.localMllib.linalg.Vector
import org.apache.spark.localMllib.stat.correlation.CorrelationNames
import org.apache.spark.localMllib.stat.{Statistics => OldStatistics}
...
object Correlation {
   ...
   */
  @Since("2.2.0")
  def corr(dataset: DataFrame, column: String, method: String): DataFrame = {
    val rdd = dataset.select(column).rdd.map {
      case Row(v: Vector) => v //OldVectors.fromML(v)
    }
    ...
    dataset.sqlContext.createDataFrame(Seq(Row(oldM)).asJava, schema) //.asML
  }
   ...
   */
  @Since("2.2.0")
  def corr(dataset: DataFrame, column: String): DataFrame = {
