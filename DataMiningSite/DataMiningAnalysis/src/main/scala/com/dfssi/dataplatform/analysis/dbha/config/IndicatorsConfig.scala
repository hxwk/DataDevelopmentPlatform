...
object IndicatorsConfig {

  final val xmlFileName: String = "RateIndicatorConfig.xml"
  final val xmlResourcePath: String = "com/dfssi/dataplatform/analysis/dbha/config"
  ...
  val DBHA_RESOURCE_EVENT_NAME_ID: Map[String, Int] = {
    val eventNameIdMap = mutable.Map[String, Int]()

    (getEventsEl(resourceXmlConfig) \ DBHA_DEF_ELEMENT_EVENT).foreach{ node =>
      val e = node.asInstanceOf[Elem]
      eventNameIdMap.put(getAttrValue(e, DBHA_DEF_ELEMENT_NAME), getAttrValue(e, DBHA_DEF_ELEMENT_ID).toInt)
    }

    eventNameIdMap.toMap
  }
