...
/**
  ...
  *
  * @author LiXiaoCong
  * @version 2018/3/1 15:52 
  */
object Terminal0705ToEsFromKafkaForOozie extends OozieAppTrait {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val consumeGroup = inputConfig.getConfigItemValue("consumeGroup", "Terminal0705ToEsFromKafka")
    val offset = inputConfig.getConfigItemValue("offset", "smallest")

    //读取处理配置
    val processConfig = config.getProcesssConfig.get(0)
    val appName = processConfig.getConfigItemValue("appName", "Terminal0705ToEsFromKafka")
