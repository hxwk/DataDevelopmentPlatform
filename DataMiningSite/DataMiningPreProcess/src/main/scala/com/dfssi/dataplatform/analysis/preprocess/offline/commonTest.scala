...
object commonTest extends AnalysisService {
	def main(args:Array[String]):Unit = {
		val SPARK_OFFLINE_ANALYSIS_DEF_FILE_NAME = "kmeansAlgTest.xml"
		algTest(SPARK_OFFLINE_ANALYSIS_DEF_FILE_NAME,"KmeansXMLTest")
	}

	def algTest(xmlFileName:String,appName:String):Unit = {
		...
		val sparkTaskDefEl = getSparkTaskDef(nodeName,appPath,xmlFileName)
		val conf = new SparkConf()
				.setAppName(getSparkAppName(sparkTaskDefEl).getOrElse(appName))
				.setMaster("local")
