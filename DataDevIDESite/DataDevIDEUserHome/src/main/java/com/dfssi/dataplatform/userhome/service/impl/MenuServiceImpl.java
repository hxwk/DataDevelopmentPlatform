...
public class MenuServiceImpl implements MenuService {
    ...
    public PageResult<MenuEntity> selectByMenuName(PageParam pageParam, String menuName) {
        ...
        for(MenuEntity menuEntity : menuEntityList){
            if(StringUtils.isNotEmpty(menuEntity.getPid())){
                menuEntity.setParentMenuName(menuDao.selectByPrimaryKey(menuEntity.getPid()).getMenuName());
            }
        }
    ...
    public String modifyMenu(MenuEntity record) {
        ...
        try {
            ...
            if(StringUtils.isEmpty(record.getPid())){
                record.setPid("");
            }
    ...
    public String insert(MenuEntity record) {
        String result="";
            ...
            if(count!=0){
                ...
                int countUpdate = menuDao.countByUpdate(record);
                if (countUpdate != 0) {
                    result = "菜单名已存在";
                } else {
...
//    }
    ...
    @Override
    public List<Object> getAllMenuNew() {
        List list=new ArrayList();
        MenuEntity entity=new MenuEntity();
        List<DicItemEntity> listSystem=dicItemService.listSelectSystem();
        for(DicItemEntity dicItemEntity:listSystem) {
            if(dicItemEntity!=null) {
                list.add(dicItemEntity);
            }
            if(StringUtils.isNotEmpty(dicItemEntity.getValue())){
                List<MenuEntity> listMenu=menuDao.selectBySystem(dicItemEntity.getValue());
                for(MenuEntity menuEntity:listMenu) {
                    if(menuEntity!=null) {
                        list.add(menuEntity);
                    }
                    if(StringUtils.isNotEmpty(menuEntity.getId())){
                        List<MenuEntity> listCmenu=menuDao.selectByParentMenu(menuEntity.getId());
                        for(MenuEntity childMenu:listCmenu){
                            if(childMenu!=null){
                                list.add(childMenu);
                            }
                        }
